...
data$pfextent2 <- factor(data$pfextent2, levels = c("Unfrz", "Isol", "Spora", "Discon", "Cont"))
...
attach(subset_data)
cuzickTest(subset_data$lf,subset_data$pfextent2, "greater")
...
attach(subset_data)
cuzickTest(lake_mean,pfextent2, "two.sided")

ggplot(data, aes(y = dens, x = pfextent2)) +
...
subset_data <- data[data$pd == 'unglaciated_fine', ]
attach(subset_data)
cuzickTest(ls,pfextent2,continuity=TRUE, "two.sided")
...
attach(s)
cuzickTest(ls,pfextent2,"two.sided")
...
coarse[is.na(coarse$pfextent2),] <- "Unfrz"
cuzickTest(coar$density, coar$pfextent2, "two.sided")
